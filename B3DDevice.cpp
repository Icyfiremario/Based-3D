...
B3DDevice::B3DDevice(B3DWindow& window) : window{window}
{
	createInstance();
	setupDebugMessenger();
	createSurface();
...
void B3DDevice::createInstance()
{
	...
	appInfo.pApplicationName = "BasedEngine3D App";
...
void B3DDevice::setupDebugMessenger()
{
...
void B3DDevice::createSurface() { window.createWindowSurface(instance, &surface_); }

void B3DDevice::pickPhysicalDevice()
{
...
{
	QueueFamilyInices indices = findQueueFamilies(physicalDevice);
...
void B3DDevice::createCommandPool()
{
	QueueFamilyInices queueFamilyIndices = findPhysicalQueueFamilies();
...
bool B3DDevice::isDeviceSuitable(VkPhysicalDevice device)
{
	QueueFamilyInices indices = findQueueFamilies(device);
...
std::vector<const char*> B3DDevice::getRequiredExtensions()
{
...
bool B3DDevice::checkValidationLayerSupport()
{
	uint32_t layerCount;
...
QueueFamilyInices B3DDevice::findQueueFamilies(VkPhysicalDevice device)
{
...
void B3DDevice::populateDebugMessangerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo)
{
	createInfo = {};
...
void B3DDevice::hasGlfwRequiredInstanceExtensions()
{
...
bool B3DDevice::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
SwapChainSupportDetails B3DDevice::querySwapChainSupport(VkPhysicalDevice device)
{
...
uint32_t B3DDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
VkCommandBuffer B3DDevice::beginSingleTimeCommands()
{
...
void B3DDevice::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
...
void B3DDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
void B3DDevice::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, uint32_t layerCount)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
